...
class BusinessController extends ApiBaseController {
	...
	public function getUserBusiness() {
		...
		if ( isset( $user->business ) ) {
			if ( $user->business->need_refresh ) {
				SyncBusiness::dispatch( $user->business );
				UpdateBusinessesNeedRefreshStatus::dispatch( 0 , $user->business);
			}
		}
	...
	public function saveUserBusiness( UpdateFormValidation $request ) {
		...
		$this->unlockBusinessOptions( $business, $business_option->level_id );
